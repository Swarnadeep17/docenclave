...
import * as pdfjs from 'pdfjs-dist'; // Keep consistent alias
import { useDropzone } from 'react-dropzone'; // <--- ENSURE THIS LINE IS PRESENT AND CORRECT
...
export default function CompressTool() {
  ...
  const [previewImageSrc, setPreviewImageSrc] = useState(null); // This will hold the dataURL of the original preview
  
  const analysisData = useRef({
    pageCount: 0,
    // Store the dataURL size of a sample page rendered as JPG at different qualities
    samplePageJpegSizeAtLowQ: 0,  // e.g., quality 10
    samplePageJpegSizeAtHighQ: 0, // e.g., quality 95
  });
  ...
  const handleFileChange = (event) => {
    ...
    if (selectedFile && selectedFile.type === 'application/pdf') {
        ...
        setPreviewImageSrc(null); // Clear old preview
        setStats({ originalSize: 0, estimatedSize: 0, reduction: 0 }); // Reset stats
    } else if (selectedFile) {
        alert('Please select a valid PDF file.');
    }
  };

  // --- Effect for Initial File Analysis & Preview Generation (runs when 'originalFile' changes) ---
  useEffect(() => {
    ...
    const analyzeAndPreview = async () => {
      ...
      try {
        ...
        const page = await pdfjsDoc.getPage(1); // Always use page 1 for consistency
        const viewport = page.getViewport({ scale: 1.0 }); // Standard scale for previews
        
        // Generate the master PNG preview for display
        ...
        // Generate JPGs at low and high quality for estimation baseline
        const estimationCanvas = document.createElement('canvas');
        estimationCanvas.height = viewport.height; // Use same viewport for consistency
        estimationCanvas.width = viewport.width;
        const estimationCtx = estimationCanvas.getContext('2d');
        await page.render({ canvasContext: estimationCtx, viewport }).promise; // Re-render needed
  ...
  // --- UNIFIED Effect for INSTANTLY Updating Stats (NO DEBOUNCE for slider changes) ---
  useEffect(() => {
    ...
    if (pageCount > 0 && samplePageJpegSizeAtHighQ > 0) {
      // Normalize quality: if slider is 1-10, it's effectively 0 for this calc; if 95-100, it's 1.
      const qualityInput = settings.quality; // 1-100
      let qualityRatio = 0;
      if (qualityInput <= 10) {
        qualityRatio = 0;
      } else if (qualityInput >= 95) {
        qualityRatio = 1;
      } else {
        qualityRatio = (qualityInput - 10) / (95 - 10);
      }
      ...
      if (settings.isGrayscale) estimatedPageSize *= 0.7; // Grayscale factor
      
      const estimatedTotalSize = estimatedPageSize * pageCount;
      const originalFileSize = originalFile.size; // Use originalFile.size
      const reduction = originalFileSize > 0 ? 100 - (estimatedTotalSize / originalFileSize) * 100 : 0;
      
      setStats({ 
          originalSize: originalFileSize, 
          estimatedSize: estimatedTotalSize, 
          reduction: Math.max(0, Math.min(100, Math.round(reduction))) // Clamp reduction between 0 and 100
      });
    }
  ...
  const handleCompress = async () => {
    ...
    try {
        ...
        for (let i = 1; i <= sourcePdf.numPages; i++) {
            ...
            const viewport = page.getViewport({ scale: 2.0 }); // Higher scale for final output
  ...
  const formatBytes = (bytes, decimals = 2) => {
    if (!+bytes || bytes < 0) return '0 Bytes'; // Handle negative if calculation is off
    const k = 1024;
  ...
  const SettingsAndPreviewView = () => (
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="md:col-span-2 bg-gray-900/50 rounded-lg p-4 flex flex-col items-center justify-center min-h-[400px] text-center">
            ...
            {previewImageSrc ? (
                <img src={previewImageSrc} alt="Original PDF Preview" className={`max-w-full max-h-[320px] object-contain border border-gray-700 rounded transition-filter duration-300 ${settings.isGrayscale ? 'grayscale' : 'grayscale-0'}`} />
            ) : (
                isProcessing && processingMessage ? <p className="text-accent">{processingMessage}</p> : <p className="text-gray-500">Preview will appear after analysis.</p>
            )}
          </div>
          <div className="md:col-span-1 flex flex-col space-y-6">
              <h3 className="text-2xl font-bold border-b border-gray-600 pb-2">Compression Settings</h3>
              <div className="text-sm text-gray-400">File: {originalFile.name}</div>
  ...
  return (
    <div className="w-full max-w-6xl mx-auto py-24 px-4">
      <ToolPageHeader title="PDF Compressor" description="Securely compress your PDF with real-time feedback and control." />
      ...
      <div className="mt-20 text-gray-300 prose prose-invert max-w-none prose-p:text-gray-300 prose-h2:text-gray-100 prose-h3:text-gray-200 prose-h4:text-gray-200">
        ...
        <p>DocEnclave's PDF Compressor empowers you to significantly reduce file sizes without sacrificing clarity, all while ensuring your data remains 100% private. Our tool operates entirely in your browser, meaning your sensitive documents are never uploaded to any server.</p>
        <h3 className="text-2xl font-bold mt-12 mb-4">Instant Feedback, Informed Decisions</h3>
        <p>As soon as you upload your PDF, you'll see an initial preview and real-time estimates of the compressed file size as you adjust settings. Choose your image quality, opt for grayscale conversion for maximum space saving, or remove unnecessary metadata. See the impact of your choices instantly before you commit to compression.</p>
        <h3 className="text-2xl font-bold mt-12 mb-4">Reliable Compression, Uncompromised Privacy</h3>
        <p>Our advanced "Reconstruction" method rebuilds your PDF page by page, applying your chosen settings to each image. This robust process ensures reliable compression even for complex files, while the client-side operation guarantees your documents never leave your computer. The result? A smaller, shareable PDF without compromising your privacy.</p>
        <h2 className="text-3xl font-bold mt-16 mb-8">Frequently Asked Questions</h2>
        <div className="space-y-8">
          <div><h4 className="text-xl font-semibold">How does the compression work?</h4><p>After you upload a PDF and choose your settings (like image quality and grayscale), our tool intelligently rebuilds the document. Each page is effectively converted into a high-quality image, which is then compressed to your specifications and reassembled into a new, smaller PDF.</p></div>
          <div><h4 className="text-xl font-semibold">Will my text still be selectable after compression?</h4><p>Because our method converts each page into an image to achieve reliable compression, the text in the final PDF will not be selectable. This is a trade-off for achieving significant and consistent file size reduction across all PDF types.</p></div>
          <div><h4 className="text-xl font-semibold">Is this tool free and private?</h4><p>Absolutely. Like all DocEnclave tools, the PDF Compressor is 100% free to use, with no watermarks or registration. All processing happens in your browser, so your files remain completely private and secure on your own device.</p></div>
        </div>
      </div>
    </div>
  );
