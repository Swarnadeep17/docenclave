...
export default function CompressTool() {
  ...
  // Effect for file analysis and preview generation
  useEffect(() => {
  ...
  const handleCompress = async () => {
    ...
    try {
      ...
      for (let i = 1; i <= sourcePdf.numPages; i++) {
        ...
        const newPage = newPdfDoc.addPage([viewport.width / 2, viewport.height / 2]); // Adjust for scale
      ...
      const pdfBytes = await newPdfDoc.save({ useObjectStreams: false }); // Optimize for size
      ...
      <div className="md:col-span-2 bg-gray-900/50 rounded-lg p-4 flex flex-col items-center justify-center min-h-[400px] text-center">
        ...
        {previewImageSrc ? (
          <img
            ...
            className={`max-w-full max-h-[320px] object-contain border border-gray-700 rounded transition-filter duration-300 ${settings.isGrayscale ? 'grayscale' : 'grayscale-0'}`}
          />
        ) : (
