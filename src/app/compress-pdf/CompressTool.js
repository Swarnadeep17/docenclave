...
// Configure PDF.js worker pdfjsLib.GlobalWorkerOptions.workerSrc = //cdnjs.cloudflare.com/ajax/libs/pdf.js/${pdfjsLib.version}/pdf.worker.min.js;
...
// When file changes, generate previews and metadata useEffect(() => { if (!file) { setPreviews([]); setMetadata({}); return; } const loadPdf = async () => { setIsProcessing(true); const buffer = await file.arrayBuffer();
...
// Extract metadata via pdf-lib
  ...
  // Generate page previews via pdfjs
...
const handleFileChange = (e) => { const selected = e.target.files?.[0]; if (selected && selected.type === 'application/pdf') { setFile(selected); setCompressedBlob(null); } };

const handleCompress = async () => { if (!file) return; setIsProcessing(true); const buffer = await file.arrayBuffer(); const loadingTask = pdfjsLib.getDocument({ data: buffer }); const pdf = await loadingTask.promise; const newPdf = await PDFDocument.create();
...
for (let i = 1; i <= pdf.numPages; i++) {
  ...
  newPage.drawImage(embedded, { x: 0, y: 0, width: dims.width, height: dims.height });
}
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="md:col-span-2">
            ...
            <div className="flex items-center mb-4">
              ...
              <input type="checkbox" checked={settings.isGrayscale} onChange={() => setSettings({ ...settings, isGrayscale: !settings.isGrayscale })} />
            </div>
