...
export default function CompressTool() {
  ...
  useEffect(() => {
    ...
    const analyzeFile = async () => {
      ...
      try {
        ...
        const pdfjsDoc = await pdfjs.getDocument({ data: pdfBuffer.slice(0) }).promise;
                ...
                if (xobjects?.isDict()) {
                    xobjects.entries().forEach(([key, ref]) => {
                        if (ref.isIndirect()) {
                            ...
                            if (xobject.lookup(PDFName.of('Subtype')) === PDFName.of('Image')) {
                                if (!imageRefs.has(ref.toString())) {
  ...
  useEffect(() => {
    ...
    if(settings.isGrayscale) estimatedImageSize *= 0.7;
