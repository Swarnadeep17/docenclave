...
export default function MergePdfPage() {
  ...
  const handleFileChange = async (event) => {
    // If there's a previously merged file, clear its URL to prevent memory leaks
    if (mergedFile) {
    ...
    for (let i = 0; i < selectedFiles.length; i++) {
      ...
      if (file.type !== "application/pdf") continue; // Skip non-PDF files
      ...
      try {
        const pdf = await pdfjs.getDocument({ data: fileAsArrayBuffer }).promise;
  ...
  const handleMerge = async () => {
    ...
    try {
      ...
      alert("An error occurred while merging the pages.");
    } finally {
      setIsProcessing(false);
    }
  };
  ...
  const handleStartOver = () => {
    ...
    // Also reset the file input so the same files can be re-selected
    ...
    <div className="text-center py-8">
      ...
      <div className="flex flex-col sm:flex-row justify-center items-center space-y-4 sm:space-y-0 sm:space-x-4">
        ...
        <button
          ...
          className="w-full sm:w-auto bg-gray-600 text-white font-bold py-3 px-8 rounded-lg hover:bg-gray-500 transition-colors"
        >
          Merge More Files
        </button>
      </div>
    ...
    <div className="w-full max-w-6xl mx-auto py-24 px-4 sm:px-6 lg:px-8">
      <div className="text-center mb-12">
        ...
        <p className="text-lg text-gray-400">
          Drag & drop pages to reorder, delete unwanted pages, then merge.
        </p>
      </div>
          ...
          <>
              ...
              <Droppable droppableId="pages" direction="horizontal">
                {(provided) => (
                  <div
                    ...
                    className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-7 gap-4 p-4 bg-gray-900/50 rounded-lg overflow-x-auto min-h-[150px]"
                  >
                      ...
                      <Draggable key={page.id} draggableId={page.id} index={index}>
                        {(provided) => (
                          <div
                            ...
                            className="relative group aspect-[7/10]"
                          >
                            <img src={page.imgSrc} alt={`Page ${index + 1}`} className="w-full h-full object-contain border-2 border-gray-600 rounded-md" />
                            <button 
                                onClick={() => handleDeletePage(page.id)}
                                className="absolute top-0 right-0 -mt-2 -mr-2 bg-red-600 text-white rounded-full w-6 h-6 flex items-center justify-center text-xs opacity-0 group-hover:opacity-100 transition-opacity z-10"
            ...
            <div className="mt-8 text-center">
                <button 
                  ...
                  className="bg-accent text-white font-bold py-3 px-8 rounded-lg hover:bg-blue-600 transition-colors disabled:bg-gray-600"
                >
