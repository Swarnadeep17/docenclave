...
// Track specific tool usage without download
export const trackToolUsage = async (toolName) => {
  try {
    ...
    console.log('Tool usage tracking failed:', error)
    // Fail silently to not disrupt user experience
  }
}
...
// Get most popular tools from current month
export const getPopularTools = async (limit = 5) => {
  try {
    const stats = await getMonthlyStats()
    const toolsUsed = stats.tools_used || {}
    ...
    console.log('Failed to get popular tools:', error)
    return []
  }
}

// Check if it's a new visitor (based on localStorage)
export const isNewVisitor = () => {
  const hasVisited = localStorage.getItem('docenclave_visited')
  if (!hasVisited) {
    localStorage.setItem('docenclave_visited', Date.now().toString())
    return true
  }
  return false
}

// Check if visitor has already been tracked this session
export const hasTrackedThisSession = () => {
  return sessionStorage.getItem('visitor_tracked') === 'true'
}

// Mark visitor as tracked for this session
export const markVisitorTracked = () => {
...
export const getConversionRate = async () => {
  try {
    ...
    return Math.round((stats.downloads / stats.visitors) * 100)
  } catch (error) {
    console.log('Failed to calculate conversion rate:', error)
    return 0
  }
}

// Reset analytics for testing (only use in development)
export const resetCurrentMonthStats = async () => {
  if (process.env.NODE_ENV !== 'development') {
    console.warn('Reset function only available in development')
    return
  }
